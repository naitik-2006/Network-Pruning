...
def greedy_decode(model, image, max_len, start_symbol = 1):
    model.eval()
    device = next(model.parameters()).device
    
    # Encode the image
    image = image.to(device)
    features = model.pruned_image_encoder(image)
    features = model.encoder(features)
    
    # Initialize the input with the start symbol
    ys = torch.ones(1, 1).fill_(start_symbol).type(torch.long).to(device)
    
    for _ in range(max_len-1):
        out = model.decoder(features, ys)
        prob = out[-1, :].squeeze(0)
        _, next_word = torch.max(prob, dim=-1)
        next_word = next_word.item()
        
        ys = torch.cat([ys, torch.ones(1, 1).type_as(ys.data).fill_(next_word)], dim=0)
        
        if next_word == model.decoder.embedding.num_embeddings - 1:  # Assuming the last token is the <EOS> token
            break
        if(next_word == 2) or (next_word == "<EOS>"):
            break
...
# Example usage:
# Assuming you have a trained model, an input image, and the <SOS> index:
# model = EncoderToDecoder()
# image = ...  # Your input image tensor
# start_symbol = 1  # Typically <SOS> token index
# max_len = 50  # Maximum length of the generated caption

# decoded_sequence = greedy_decode(model, image, max_len, start_symbol)
...
def run_validation(model, validation_dataloader, validation_dataset, max_len, device, writer):
    ...
    with torch.no_grad():
        for idx , (image , caption) in enumerate(validation_dataloader):
            ...
            #model_out_2 = model_out.squeeze(0).detach().cpu().numpy().tolist()
            #model_out_flat = [token for sublist in model_out_2 for token in sublist]
            ...
            #print("Target Text:", target_text_2)
            #print("Model Output Text:", model_out_text)

            # Display the image and its corresponding target caption
            #imshow(image[0].cpu(), title="Target: " + target_text_2)
...
if __name__ == "__main__":
    ...
    model = torch.load(r'Models\model_final_4_1_T.pth')
